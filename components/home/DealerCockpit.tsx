...
export default function DealerCockpit({
  ...
  return (
    <div className="space-y-5 pb-4">
      ...
      <PlayingCard className="px-5 py-5">
        <div className="mb-3 flex items-start justify-between gap-3">
          ...
          <button
            onClick={onEndShift}
            className="shrink-0 rounded-lg border border-td-border px-2.5 py-1.5 text-[11px] font-semibold text-td-muted hover:border-td-red hover:text-red-300"
          >
            End Shift
          </button>
        </div>
        ...
        {isTournament ? (
          ...
          <div className="mt-4 rounded-xl border border-td-border/70 bg-td-surface2/50 px-3 py-3 text-center">
            ...
            <p className="mt-1 font-mono text-[32px] font-semibold text-td-goldsoft">{fmtMoney(net)}</p>
        ...
        <div className="mt-4 h-1.5 overflow-hidden rounded-full bg-td-surface2">
          <div
      ...
      {target ? (
        <PrimaryPlayingButton onClick={() => onLogDown(target.block)}>
          {target.label}
      ...
      {target && (
        <div className="grid grid-cols-2 gap-2.5">
          <SecondaryPlayingButton onClick={() => onBreak(target.block)}>
            <Coffee size={16} /> Break
          </SecondaryPlayingButton>
          <SecondaryPlayingButton onClick={() => onSkip(target.block)}>Skip</SecondaryPlayingButton>
        </div>
      )}

      {!isTournament && (
        <SecondaryPlayingButton onClick={onLogLumpSum}>
          <PenLine size={16} />
      ...
      {!extendOpen ? (
        <SecondaryPlayingButton onClick={() => setExtendOpen(true)}>
          <Plus size={16} /> Add More Time
        </SecondaryPlayingButton>
      ) : (
        <PlayingCard className="space-y-2 px-4 py-4">
          ...
          <div className="grid grid-cols-3 gap-2">
            {[60, 120, 240].map((mins) => (
              <button
                ...
                className="rounded-xl border border-td-border bg-td-surface2 py-2.5 text-[13px] font-semibold text-td-cream hover:border-td-gold"
              >
                {mins < 120 ? `${mins}m` : `${mins / 60}h`}
              </button>
            ))}
          </div>
          <button onClick={() => setExtendOpen(false)} className="w-full py-1 text-[12px] text-td-muted">
            Cancel
          </button>
        </PlayingCard>
      )}
      ...
      <div className="space-y-2 pt-1">
        <p className="text-[10px] font-semibold uppercase tracking-[1.5px] text-td-muted">All Downs</p>
        {shift.blocks.map((b) => (
